...
function useDashboardData() {
  const fetchAll = useCallback(async () => {
    ...
    return {
      summary,
      websites: asArray(websites).map(normalizeWebsite),
      apps: asArray(apps),
    };
  }, []);
...
function DashboardContent() {
  ...
  const down =
    summary.down ?? websites.filter((w) => w.status === "down").length;
  const appsCount =
    summary.businessApps?.total ??
    summary.totalBusinessApps ??
    data?.apps?.length ??
    0;
  return (
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500">
            Live status of everything you watch.
          </p>
        </div>
        <motion.button
          whileTap={{ scale: 0.98 }}
        ...
        <>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <StatCard
              label="Down"
              value={down}
              tone={down > 0 ? "red" : "zinc"}
              sub={down > 0 ? "Needs attention" : "All quiet"}
            />
            <StatCard
              label="Business apps"
              value={appsCount}
            ...
            <EmptyState
              ...
              hint={
                q
                  ...
                  : "Add your first website and checks start within a minute."
              }
              action={
                !q ? (
                  <Link
                  ...
                  <tbody>
                    {websites.map((w, i) => (
                      <motion.tr
                        ...
                        className={cx(
                          "border-t border-zinc-100 transition hover:bg-zinc-50",
                          w.id && "cursor-pointer",
                        )}
                      >
                        ...
                        <td className="px-4 py-3 text-right tabular-nums text-zinc-600">
                          {formatMs(w.responseMs)}
                        </td>
                        <td className="px-4 py-3 text-right tabular-nums text-zinc-600">
                          {formatPct(w.uptime)}
                        </td>
                        <td className="px-4 py-3 text-right">
                          <span className="text-zinc-900 underline-offset-2 hover:underline">
                            Open
                          </span>
                        </td>
                      </motion.tr>
